            ...
            <div className="bg-white rounded-lg w-full max-w-7xl h-[90vh] flex flex-col">
              <div className="p-6 border-b border-gray-200">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Button onClick={() => setShowQuestionBank(false)} variant="outline">
                    <X className="w-4 h-4" />
                  </Button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-4">
                  ...
                  <select
                    ...
                    onChange={(e) => setSelectedQuestionUnit(e.target.value)}
                    className="px-3 py-2 border border-gray-300 rounded-md"
                  >
                    <option value="all">All Units</option>
                    {subject.curriculum.curriculumDocument?.extractedUnits?.map((unit, index) => (
                  ...
                  <select
                    ...
                    className="px-3 py-2 border border-gray-300 rounded-md"
                    disabled={selectedQuestionUnit === "all"}
                  >
                    <option value="all">All Topics</option>
                    {selectedQuestionUnit !== "all" && subject.curriculum.curriculumDocument?.extractedUnits?.[parseInt(selectedQuestionUnit)]?.topics.map((topic, index) => (
                      <option key={index} value={index.toString()}>
                        {topic.topicName}
                      </option>
                    ))}
                  ...
                  <select
                    value={selectedQuestionSubtopic}
                    onChange={(e) => setSelectedQuestionSubtopic(e.target.value)}
                    className="px-3 py-2 border border-gray-300 rounded-md"
                    disabled={selectedQuestionTopic === "all"}
                  >
                    <option value="all">All Subtopics</option>
                    {selectedQuestionUnit !== "all" && selectedQuestionTopic !== "all" &&
                     subject.curriculum.curriculumDocument?.extractedUnits?.[parseInt(selectedQuestionUnit)]?.topics?.[parseInt(selectedQuestionTopic)]?.subtopics.map((subtopic, index) => (
                      <option key={index} value={index.toString()}>
                        {subtopic}
                      </option>
                    ))}
                  ...
                  <div className="text-sm text-gray-600 flex items-center">
                    ...
                    {filteredQuestions.length} of {generatedQuestions.length} questions
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                  <div className="bg-blue-50 p-3 rounded-lg">
                    <div className="text-lg font-bold text-blue-600">{generatedQuestions.length}</div>
                    <div className="text-xs text-blue-600">Total Questions</div>
                  </div>
                  <div className="bg-green-50 p-3 rounded-lg">
                    <div className="text-lg font-bold text-green-600">
                      {subject.curriculum.curriculumDocument?.extractedUnits?.reduce((acc, unit) =>
                        acc + unit.topics.reduce((topicAcc, topic) => topicAcc + topic.subtopics.length, 0), 0) || 0}
                    </div>
                    <div className="text-xs text-green-600">Subtopics Covered</div>
                  </div>
                  <div className="bg-purple-50 p-3 rounded-lg">
                    <div className="text-lg font-bold text-purple-600">250</div>
                    <div className="text-xs text-purple-600">Questions per Subtopic</div>
                  </div>
                  <div className="bg-orange-50 p-3 rounded-lg">
                    <div className="text-lg font-bold text-orange-600">5</div>
                    <div className="text-xs text-orange-600">Options per Question</div>
                  </div>
                </div>
              </div>
                    ...
                    <Card key={question.id} className="border border-gray-200">
                      <CardContent className="p-4">
                        ...
                        <div className="mb-3">
                          ...
                          <p className="text-sm text-gray-600 bg-blue-50 p-3 rounded border">{question.explanation}</p>
                        </div>

                        {/* Working Steps */}
                        <div>
                          <Label className="text-sm font-medium text-gray-700 mb-1 block">Step-by-Step Solution:</Label>
                          <div className="bg-yellow-50 p-3 rounded border">
                            <ol className="text-sm text-gray-600 space-y-1">
                              {question.workingSteps.map((step, index) => (
                                <li key={index} className="flex">
                                  <span className="font-medium text-yellow-700 mr-2">{index + 1}.</span>
                                  <span>{step}</span>
                                </li>
                              ))}
                            </ol>
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-600">
                        {generatedQuestions.length === 0
                          ? "Generate questions first to view them here."
                  ...
                  <div className="flex space-x-3">
                    ...
                    <Button
                      onClick={() => {
                        ...
                        a.href = url;
                        a.download = `question-bank-${subject.slug}.json`;
                        a.click();
                      }}
                      className="bg-study-primary hover:bg-study-primary/90"
                    >
