...
// Flashcard curriculum mapping
interface FlashcardCurriculumInfo {
  unit: string;
  topic: string;
  subtopic?: string;
}

const flashcardCurriculumMapping: Record<string, FlashcardCurriculumInfo> = {
  ...
  return (
    <div className="min-h-screen bg-study-background">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Current Flashcard Topic */}
        {flashcardCurriculumMapping[slug as string] && (
          <Card className="mb-8 border-sky-blue-200">
            <CardContent className="p-4">
              <div className="flex flex-wrap items-center gap-2 text-sm">
                <span className="text-gray-600 font-medium">Currently studying:</span>
                <Link to={`/subjects/${slug}#unit-${getUnitNumber(flashcardCurriculumMapping[slug as string].unit)}`}>
                  <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200 hover:bg-blue-100 cursor-pointer transition-colors">
                    {flashcardCurriculumMapping[slug as string].unit}
                  </Badge>
                </Link>
                <ChevronRight className="w-3 h-3 text-gray-400" />
                <Link to={`/subjects/${slug}#topic-${getTopicId(flashcardCurriculumMapping[slug as string].topic)}`}>
                  <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 hover:bg-green-100 cursor-pointer transition-colors">
                    {flashcardCurriculumMapping[slug as string].topic}
                  </Badge>
                </Link>
                {flashcardCurriculumMapping[slug as string].subtopic && (
                  <>
                    <ChevronRight className="w-3 h-3 text-gray-400" />
                    <Link to={`/subjects/${slug}#subtopic-${getSubtopicId(flashcardCurriculumMapping[slug as string].subtopic!)}`}>
